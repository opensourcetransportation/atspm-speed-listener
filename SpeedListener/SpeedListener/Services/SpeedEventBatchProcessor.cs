    ...
    ILogger<SpeedEventBatchProcessor> logger) : ISpeedEventBatchProcessor
{
    ...
    private async Task FlushAsync(List<SpeedEvent> batch, CancellationToken cancellationToken)
    {
        ...
        foreach (var group in batch.GroupBy(speedEvent => speedEvent.DetectorId, StringComparer.OrdinalIgnoreCase))
        {
            ...
            {
                logger.LogWarning("No speed-sensor device mapping exists for detector {DetectorId}; dropping {Count} events",
                    detectorId, group.Count());
                continue;
            }
        ...
        if (envelopes.Count > 0)
            await publisher.PublishAsync(envelopes, options.Value.ArchiveParallelism, cancellationToken);
